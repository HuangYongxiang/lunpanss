...
import com.example.xianlong.lunpan.activity.InfoActivitys;
import com.example.xianlong.lunpan.activity.MoneyActivity;
import com.example.xianlong.lunpan.activity.SplashActivity;
...
public class MainActivity extends BaseActivity implements LuckPanLayout.AnimationEndListener,AMapLocationListener{
    ...
    protected  void tongzhi3(){
        ...
        ((TextView) view.findViewById(R.id.alertdialog_option_one_content))
                .setText("前期漏洞数据正在去除，有无意多转的用户,请提前告知情况，以免被封闭账号。另因摘除数据，和增加广告原因，停转三天");
        ...
        alertdialog_option_two_right.setText("知道了");
        // alertdialog_option_two_right.setVisibility(View.GONE);
    ...
    protected void indata(){
        ...
        params.put("turntableTaskTrialImei", Utils.getImei(MainActivity.this));
        params.put("turntableTaskTrialIp", Utils.getip(MainActivity.this));
                ...
                .execute(new GenericsCallback<BeanResult>(new JsonGenericsSerializator())
                {
                    ...
                    public void onResponse(final BeanResult response, int id)
                    {
                        if(response.message.equals("请更换手机IMEI")){
                            isok=true;
                            dismissLoadingDialog();
                            ToastUtil.show(MainActivity.this, "请更换手机");
                        }else {
    ...
    public void endAnimation(int position) {
        int a =position+1;
        if(position==9){
            //Toast.makeText(this,"Position = "+a+","+strs[0],Toast.LENGTH_SHORT).show();
        }else{
            //Toast.makeText(this,"Position = "+a+","+strs[a],Toast.LENGTH_SHORT).show();
        }
        if(neirongs.get(position).turntableTaskTypeId.equals("1")){
            isok=true;
            tongzhi1(neirongs.get(position).turntableTaskMoney);
            //indat(Utils.getImei(MainActivity.this));
            idred=true;
            intdata1();
        }else{
            isok=true;
    ...
    protected  void intdata1(){
        ...
        String b = Utilss.getip(MainActivity.this);//ip
        String c = Utilss.getMacAddress();//mac地址
        String d= Utilss.getMacAddr();//mac地址
        int e= IntenetUtil.getNetworkState(MainActivity.this);//网络类型
        String network=e+"";
        if(e==1){
            network="wifi";
        }else{
        ...
        float xdpi = MainActivity.this.getResources().getDisplayMetrics().xdpi;
        float ydpi =  MainActivity.this.getResources().getDisplayMetrics().ydpi;
        ...
        //String sd=x1+","+y1;
        String g= Utilss.getImei(MainActivity.this);//imei
        String ts= Utilss.getTime();//imei
        WindowManager wm = MainActivity.this.getWindowManager();
        int width = wm.getDefaultDisplay().getWidth();
        int height = wm.getDefaultDisplay().getHeight();
        String sss=Utilss.getIsmi(MainActivity.this);
      /*  if(Utilss.getIsmi(MainActivity.this)==null){
        ...
        params.put("imsi",Utilss.getIsmi(MainActivity.this));//运营商识别码	例如:460011418603055
        params.put("ua",Utilss.getua(MainActivity.this));//浏览器的user-agent
        ...
        params.put("androidid",Utilss.getAndroidID(MainActivity.this));//安卓id	仅安卓机型提供
                        ...
                        if(response==null){
                            ...
                            if (response.state == null) {
                                ...
                                if (response.state.equals("500")) {
                                    ...
                                    ToastUtil.show(MainActivity.this, "获取广告异常");
                                } else {
                                    //成功
                                    displayreport=response.displayreport;
                                    queding();
